...
Die Einstellung `Service.Think: "false"` hält das Reasoning des Modells aus Captions und Kategorien heraus. Viele aktuelle Modelle sind Thinking‑ bzw. Reasoning‑Modelle, und bei aktiviertem Reasoning geben neuere Ollama‑Versionen dieses im Ergebnis aus (Captions beginnen mit Text wie *"The user wants a concise description of the provided image…"* und das Label‑JSON lässt sich nicht mehr parsen). Ab dem kommenden Release deaktiviert PhotoPrism das Ollama‑Reasoning standardmäßig, sodass dies dort eher eine Absicherung als eine Voraussetzung ist – auf aktuellen und älteren Versionen ist es jedoch erforderlich und überall harmlos. Aktiviere Reasoning nur bewusst wieder mit `Service.Think: "true"`.
