...
| `Think`                            | `""`           | Reasoning-Hint für Ollama; als String übergebene Werte (`"true"`/`"false"`) werden als JSON-Booleans gesendet. Setze `"false"` für Thinking-Modelle (Qwen3.5, `qwen3-vl:*`, …), damit Reasoning nicht in Captions/Kategorien ausgegeben wird. |
