...
Konten, für die die Anmeldung mit `--no-login` deaktiviert ist, können sich weder an der Benutzeroberfläche anmelden noch Anfragen an die [REST API](https://docs.photoprism.app/developer-guide/api/) authentifizieren, auch nicht mit einem [App-Passwort](client-credentials.md#app-passworter). In Kombination mit `--webdav` bleiben sie für die [WebDAV](../sync/webdav.md)-Synchronisierung mit App-Passwörtern nutzbar, die die [Berechtigung](client-credentials.md#berechtigungen) `webdav` haben. Anwendungen, die ohne Benutzeranmeldung auf die REST API zugreifen müssen, können ein [Access-Token](client-credentials.md#access-token) oder [Client-Zugangsdaten](client-credentials.md#client-zugangsdaten) verwenden.
...
!!! tldr ""
    Führe `photoprism audit reset --yes` aus, um alle aufgezeichneten Einträge zu löschen und die Datenbank auf einen sauberen Zustand zurückzusetzen. Das lässt sich nicht rückgängig machen, erstelle deshalb vorher ein [Backup](../backups/index.md), wenn du sie behalten musst.
...
Um anderen Anwendungen und Diensten eingeschränkten API-Zugang zu gewähren, kannst du mit dem Befehl `photoprism clients add` [OAuth2-Client-Zugangsdaten](client-credentials.md#client-zugangsdaten) für sie generieren. Den Befehl `photoprism auth add` kannst du verwenden, um [Access-Token](client-credentials.md#access-token) mit [eingeschränkter Berechtigung](client-credentials.md#berechtigungen) und einer begrenzten Lebensdauer zu generieren.
